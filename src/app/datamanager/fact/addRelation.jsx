...
export default class AddRelation extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            relateAssets: [],
            rightAssets: [],
            assetInfo: { assetsId: this.props.params.id },
            leftColumns: [],
            rightColumns: [],
        }
    }
    ...
    getLeftColumns = async () => {
        let query = {
            ...
            modelAssetsId: this.props.params.id,
        }
                                ...
                                <Row>
                                    {this.props.canEdit ? null : (
                                        <Col span={24} style={{ marginBottom: 0 }}>
                                            <span style={{ marginRight: 43 }}>请在当前维度资产中选择需要被关联的维度</span>
                                            <span>添加关联维度信息</span>
                                        </Col>
                                    )}
                                    <Col span={24}>
                                        <Select placeholder='请选择维度信息' style={{ width: 266 }} disabled={this.props.canEdit} value={assetInfo.leftBusinessId} onChange={this.onChangeLeftAsset}>
                                            {relateAssets.map((item) => {
                                                return (
                                                    <Option title={item.name + ' ' + item.englishName} value={item.id} key={item.id}>
                                                        {item.name}
                                        ...
                                        <span style={{ margin: '0 8px' }}>关联</span>
                                        <Select placeholder='请选择维度信息' style={{ width: 266 }} disabled={this.props.canEdit} value={assetInfo.rightBusinessId} onChange={this.onChangeRightAsset}>
                                    ...
                                    <Col span={24}>
                                        <Select placeholder='选择关联字段' style={{ width: 266, marginRight: 43 }} value={assetInfo.leftColumnId} onChange={this.onChangeLeftColumn}>
                                        ...
                                        <Select placeholder='选择关联字段' style={{ width: 266 }} value={assetInfo.rightColumnId} onChange={this.onChangeRightColumn}>
