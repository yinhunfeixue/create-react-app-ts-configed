...
export default class GaugeChart extends Component {
    ...
    setChartData = async (value) => {
        ...
        if (chart) {
            ...
            let parent = document.getElementById('dataStandard_container')
            parent.innerHTML = ''
            this.initChart()
        }
    }
    initChart = () => {
        ...
        chart = new G2.Chart({
            container: 'dataStandard_container',
            forceFit: true,
            height: 300,
            padding: [0, 0, 30, 0],
        })
        ...
        chart.guide().html({
            position: ['50%', '95%'],
            html: '<div style="width: 300px;text-align: center;">' + '<p style="font-size: 24px;color: #545454;margin: 0;">' + (this.state.chartDataValue*100).toFixed(2).replace(/[.]?0+$/g, '') + '%</p>' + '<p style="font-size: 14px;color: #333;marginTop: 16px;">字段落标率</p>' + '</div>',
        })
    ...
    render() {
        ...
        return (
            <div>
                <div id='dataStandard_container'></div>
            </div>
        )
    }
}
