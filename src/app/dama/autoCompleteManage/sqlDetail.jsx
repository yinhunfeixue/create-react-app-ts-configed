...
import { Button, Card, Divider, Form, Input, Spin, Table, Tabs, Tag, Tooltip, TreeSelect, Typography } from 'antd'
...
export default class SqlDetail extends Component {
    ...
    componentDidMount = async () => {
        await this.setState({
            queryParam: this.props.location.state !== undefined ? this.props.location.state.data : qs.parse(this.props.location.search, { ignoreQueryPrefix: true }),
        })
        this.getBasicInfo()
        // this.getLineage()
    ...
    getBasicInfo = async () => {
        ...
        let res = await getSqlBasic({ id: this.state.queryParam.id })
        if (res.code == 200) {
            this.setState({
                basicInfo: res.data,
            })
            ...
            <TableLayout
                title={
                    <div className='HControlGroup'>
                        <img style={{ width: 20 }} src={require('app_images/dataAsset/code.png')} />
                        <span>{_.get(basicInfo, 'name')}</span>
                    </div>
                }
                renderDetail={() => {
                    return (
                        <Tabs animated={false} onChange={this.tabKeyChange} activeKey={tabValue}>
                            <TabPane tab='详情' key='1'>
                                <Module title='基本信息' style={{ marginTop: 15 }}>
                                            ...
                                            },
                                            {
                                                ...
                                                content: (
                                                    <span className='ant-form-text'>
                                                        {!_.isEmpty(_.get(basicInfo, 'tagList')) ? (
                                ...
                                <Module
                                    ...
                                    title='脚本内容'
                                >
                                    <TextArea rows={15} style={{ height: '400px' }} value={_.get(basicInfo, 'sql', '-')} />
                                </Module>
                            </TabPane>
                            <TabPane tab='血缘信息' key='2'>
                                ...
                                >
                                    <Spin spinning={graphLoading}>
                                        <Graph onRef={(c) => (this.ChildTable = c)} height={500} width={graphWidth} type='sql' data={fileLineage} id={this.state.queryParam.id} />
                                    </Spin>
                                </Card>
                            </TabPane>
