...
import { getSearchData, getMetrics, getMatchData, searchMetrics, getSearchDetail, getIndexSample, searchMetricsBusiness, usingBusinessIds } from 'app_api/wordSearchApi'
import {
    // addfavouriteList,
    basicQuery,
    // deletefavouriteList,
...
class Store {
    ...
    @action.bound async onSearch (params) {
        ...
        let res = await basicQuery(params)
        ...
        if (res.code === 200) {
            runInAction(() => {
                ...
                this.unrecognized = res.data.unrecognized
    ...
    // 切换业务线
    @action.bound changeRadioValue = (ids, type) => {
        // this.businessId = id
        // this.type = type

        // let ids = id.split(',')
        ids.map((item) => {
            this.businessIdList.unshift({ id: item, type })
        })
    ...
    // 搜索方法
    @action.bound async getMetricsMethod (params) {
        let res = await getMetrics(params)
        if (res.code === 200) {
            runInAction(() => {
    ...
    @action.bound async searchMetricsMethod (params) {
        let res = await searchMetrics(params)
        if (res.code === 200) {
            runInAction(() => {
                this.metricsList = res.data
            })
