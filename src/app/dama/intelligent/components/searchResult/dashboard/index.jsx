...
import './index.less'
import { getBoardList, addBoardView, createBoard } from 'app_api/dashboardApi'
                ...
                if (selectedKey) {
                    if (this.props.handleAddBoardView) {
                        ...
                        if (data.code === 200) {
                            ...
                            NotificationWrap.success('添加成功！')
                        } else {
                            NotificationWrap.error(data.msg)
                        }
                    }
                }
            }
                        ...
                        : <div className='dashboardAdd' >
                            <Form>
                                ...
                                <Row>
                                    <Col>
                                        {
                                            loading ? <DataLoading /> : dList.length > 0 ? <div>
                                                <Row>
                                                    <Col>
                                                        <div style={{ width: '100%' }}>
                                                            <Button style={{ float: 'right', border: '0', paddingRight: '0px' }} onClick={this.visibleCreateDashboard} ><PlusCircleFilled className='colorChangeSvg toolTipIcon' /> <span className='tooTipText' >创建新的数据看板</span></Button>
                                                        </div>
                                                                        ...
                                                                        return (
                                                                            <Menu.Item className='menuItemLi' key={value.id} >
                                                                                <Row className='menuItemDetial'>
                                                                                    <Col><span>{value.id == selectedKey ? SvgChart['Right']['img'] : SvgChart['Add']['img']}</span> <span>{value.name}</span>{value.hasReports && <span className='selectTable'>报表</span>}</Col>
                                                                                </Row>
                                                                            </Menu.Item>
                                                                        )
                                                                    })
                                                ...
                                                : <Row>
                                                    <Col>
                                                        <div style={{ width: '100%', textAlign: 'center' }}><Button onClick={this.visibleCreateDashboard} type='primary'>创建看板</Button></div>
                                                    </Col>
                                                  </Row>
                                        }
                                    </Col>
