...
import { getTableList, deleteTable } from 'app_api/dashboardApi'
...
import './index.less'
import Delete from 'app_images/dataManage/删除数据.svg'
import Preview from 'app_images/dataManage/预览数据.svg'
const { Search } = Input
export default class dataManage extends Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            {
                title: '数据更新时间',
                dataIndex: 'scheduleExecTime',
                key: 'scheduleExecTime',
                className: 'centerRow'
            },
            // {
            //     title: '更新时间',
                ...
                render: (text, record) => {
                    return (
                        <div style={{ textAlign: 'center' }}>
                            <a style={{ marginRight: '8px' }} onClick={this.goDataSet.bind(this, record.id)}><img src={Preview} /></a>
                            <a onClick={this.onDel.bind(this, record.id, record.name)}><img src={Delete} /></a>
                        </div>
                    )
                }
            },
    ...
    onDelete = async () => {
        ...
        let res = await deleteTable({ id: dataId })
        if (res.code === 200) {
