...
import { getQuickTip, getMatchData, logSuggestion } from 'app_api/wordSearchApi'
...
export default class searchBlock extends Component {
    ...
    checkOption = async (value, params, dataIndex, e) => {
        ...
        if (logRes.code !== 200) {
            NotificationWrap.error(logRes.msg)
        }
    ...
    getKeyWord = async (param) => {
        ...
        let res = await getQuickTip(params)
        if (res.code === 200) {
            this.setState({
                listOption: res.data,
                visable: true
        ...
        return (
            <div contentEditable='false'>
                <div className='kwBlock'>
                    ...
                    <div className='del' onClick={this.delBlock}><CloseOutlined /></div>
                    <div
                        className='block'
                        contentEditable='true'
                ...
                <div className='selectList' onMouseDown={this.prevent}>
                    {
                        visable && listOption && listOption.length > 0 && <div className='blockList' contentEditable={false} ref={(ref) => { this['checkList'] = ref }}>
                            {
                                listOption.map((value, index) => {
                                    return (
                                        <div
                                            className='blockOpiton'
                                            contentEditable='fasle'
                                            key={index}
                                            value={value}
                                            onMouseDown={this.checkOption.bind(this, value.content, value, index)}
                                        >
                                            ...
                                            <div style={{ display: 'inline-block', position: 'relative', paddingLeft: '20px' }}>
                                                <span style={{ display: 'block', marginBottom: '-4px', fontSize: '12px' }}>
                                                    {
                                                        value.content
                                                    }
                                                </span>
