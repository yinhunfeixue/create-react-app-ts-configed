...
import { Button, Modal, Switch, Popconfirm, Row, Col, Table, Tooltip, message } from 'antd'
...
import Clear from 'app_images/清空.svg'
...
export default class Result extends Component {
        ...
        this.state = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    key: 'keywordContent',
                    render: (text, record) => (
                        <Tooltip title={text.join(', ')} placement='topLeft' >{text.join(', ')}</Tooltip>
                    ),
                    ellipsis: true,
                },
                {
                    ...
                    render: (text, record) => (
                        <Popconfirm
                            ...
                            onConfirm={() => { this.deleteView(record) }}
                            okText='确定'
                            cancelText='取消'
                        >
                            <div title='删除视图' style={{ align: 'center' }} >{SvgViewChart['DeleteViewIcon']['img']}</div>
                        </Popconfirm>
                    )
                }
            ],
    ...
    */
    ...
    clearMethod = async () => {
        ...
        store.getSearchMetricsBusiness({ businessIds: store.businessIds })
        store.clearButton()
    }
    ...
    */
        ...
        if (searchItem.length === 0) {
            if (checked) {
                if (store.businessIdList.length === 0) {
                    ...
                    this.props.tabOperate.addTab('sentenceSearch', { businessId: store.businessIds.length?store.businessIds[0]:0, businessIds: store.businessIds, tempBusinessId: store.tempBusinessId })
                    return
                }
                if (store.businessIdList.length > 0) {
                    // this.props.tabOperate.removeTab('kewordSearch')
                    this.props.tabOperate.addTab('sentenceSearch', { type: store.businessIdList[0].type, businessId: store.businessIdList[0].id, businessIds: store.businessIds, tempBusinessId: store.tempBusinessId })
                }
                // else {
    ...
    renderViewList = (viewList, operate) => {
        this.setState({
            ...
            if (operate === 'expend') {
                !this.state.viewShowStatus && this.setState({
                    viewShowStatus: true,
                    viewListOverHidden: ''
                })
                // if (clientHeight > 42) {
        ...
        return (
            <div className='searchWrap'>
                <div className='searchContent'>
                    ...
                    >
                        {/* {
                            searchItem.length === 0 && <div style={{ minWidth: '5px' }} ref={(ref) => { this['blank'] = ref }} ></div>
                        } */}
                        {
                            searchItem.map((value, index) => {
                                return (
                    ...
                    <div className='rightBtn'>
                        {
                            searchItem.length >= 1 ? <span className='clear' onClick={this.clearMethod}>
                                <img src={Clear} style={{ width: '100%' }} />
                            </span> : <span style={{ display: 'inline-block', height: 24, width: 34 }}></span>
                        }
                        <span className='del' onClick={this.searchMethod}>
                            <img src={Search} style={{ width: '100%' }} />
                        </span>
                        <span className='intell'>
                            <Switch disabled={store.type === 2} onChange={this.onSwitch} checked={checked} /> 智能取数
                        </span>
                        ...
                        ? <div className='ambiguity'>
                            {
                                ...
                                查询中的“{ambiguityList[0].content}”匹配到了多个指标。以下结果中，我们使用了 “{ambiguityList[0].name}” 进行计算
                                        <div className='warnBtn'><Button type='primary' style={{ marginRight: '8px' }} onClick={this.edit.bind(this, ambiguityList[0].dataIndex)}>修改</Button><Button onClick={this.accept.bind(this, ambiguityList[0])}>接受</Button></div>
                                    </div>
                                    : <div className='warnList'>
                                        <img src={Prompt1} className='warnLogo' />
                                没有查询到和“{ambiguityList[0].content}”相关的指标或数据
                                        <div className='warnBtn'><Button type='primary' onClick={this.edit.bind(this, ambiguityList[0].dataIndex)}>修改</Button></div>
                                    </div>
                            }
                            {/* <div style={{ lineHeight: '32px' }}>{'<' + ambiguityList.length + '>'}</div> */}
                        </div> : null
                }
                {
                    viewList.length > 0 ? <div className='searchView' ref={(dom) => { this.searchViewDom = dom }}>
                        <Row>
                            <Col span='2' >搜索视图<Tooltip title='搜索视图' className='searchViewTooltip' >{SvgViewChart['HelpIcon']['img']}<i>{SvgViewChart['HelpIcon']['hover']}</i></Tooltip></Col>
                                    ...
                                    return (
                                        <div style={{ width: '100%' }}>
                                            <div className='viewTag' >
                                                <span className={clsName} onClick={() => { this.viewTagClick(val) }}>
                            ...
                            <Col span='2'>
                                <span className='viewShowIcon' onClick={this.viewShowStatus} >
                                    {
                                        !viewShowStatus ? <span>{SvgViewChart['CollapseIcon']['img']}<i>{SvgViewChart['CollapseIcon']['hover']}</i></span> : <span>{SvgViewChart['ExpendIcon']['img']}<i>{SvgViewChart['ExpendIcon']['hover']}</i></span>
                                    }

                                </span>
                                <span onClick={this.searchViewManage} className='viewManIcon' title='搜索视图管理'>{SvgViewChart['SettingViewIcon']['img']}<i>{SvgViewChart['SettingViewIcon']['hover']}</i></span>
                            </Col>
                        </Row>
                    </div> : null
                }
                ...
                >
                    ...
                    <div style={{ textAlign: 'right' }}>
                        ...
                        <Button type='primary' onClick={this.goOnIntelligent}>确定</Button>
                    </div>
                </Modal>
                <Modal
                    // centered
                ...
                {/* <div className='searchViewTable' style={{ display: visibleView ? '' : 'none' }}>
