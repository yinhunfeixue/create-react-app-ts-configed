...
import { Button, Checkbox, Dropdown, Menu, message, Modal, Select, Spin } from 'antd'
import { bizTree, dataSecurityLevelList, delBizTree, sortNodes, suggestPath } from 'app_api/dataSecurity'
import React, { Component } from 'react'
import CategoryDetailDrawer from './component/detailDrawer'
...
export default class DataCategory extends Component {
    ...
    getBizTree = async () => {
        ...
        let res = await bizTree({ attrFilter: checked })
    ...
    getTreeData = (treeData, id) => {
        ...
        treeData.map((item) => {
            if (item.id == id) {
                if (!item.hasChild && item.businessTag == 2) {
    ...
    onClickMenu = (data, index, e) => {
        ...
        if (e.key == 1) {
            this.categoryDetailDrawer && this.categoryDetailDrawer.openModal(data)
        } else if (e.key == 2) {
    ...
    renderTreeItem = (level) => {
        ...
        const menu = (data, index) => (
            <Menu onClick={this.onClickMenu.bind(this, data, index)}>
                ...
                {PermissionManage.hasFuncPermission('/dt_calssification/def/biz/edit') && <Menu.Item key='2'>编辑</Menu.Item>}
                {PermissionManage.hasFuncPermission('/dt_calssification/def/biz/delete') && (
                    <Menu.Item key='3'>
                        ...
                        <div className={(selectedTitle[level - 1] ? selectedTitle[level - 1].name : undefined) == item.name ? 'itemDiv itemDivSelected' : 'itemDiv'}>
                            <div className='itemTitle' onClick={this.selectTree.bind(this, item)}>
                                {/* item.businessTag == 2 && item.securityLevel ?
                                            <Tag color={item.securityLevel == 1 ? 'blue' : (item.securityLevel == 2 ? 'geekblue' : (item.securityLevel == 3 ? 'purple' : (item.securityLevel == 4 ? 'orange' : 'red')))}>
                                ...
                                <Dropdown overlay={menu(item, level - 1)} placement='bottomLeft' overlayClassName='categoryMenuDropdown'>
                                    <span className='iconfont icon-more' style={{ right: !item.hasChild || item.businessTag == 2 ? '19px' : '35px' }}></span>
                                </Dropdown>
                                <span className='titleIcon'>{!item.hasChild || item.level == 3 ? null : <span className='iconfont icon-you' style={{ marginLeft: 4 }}></span>}</span>
                            </div>
    ...
    selectTree = (data) => {
        let { selectedItem, selectedTitle } = this.state
        console.log('selectedTree', selectedItem, selectedTitle)
        if (!data.hasChild && data.businessTag == 2) {
    ...
    confirmDelete = async () => {
        ...
        let res = await delBizTree({ id: this.state.delInfo.id })
    ...
    getCurrentRenderData = (level) => {
        ...
        array = array !== undefined ? array : []
        console.log('getCurrentRenderData', array)
        return array
    }
    ...
    render() {
        ...
        console.log('render', selectedItem, renderLength)
        ...
        return (
            <div className='dataCategory'>
                                ...
                                {renderLength.map((item, index) => {
                                    return (
                                        <div className={`categoryItem`}>
                                            <div className={selectedTitle[index] ? 'itemHeader' : 'itemHeader itemHeaderGrey'}>{selectedTitle[index] ? selectedTitle[index].name : '业务分类'}</div>
                                            {this.renderTreeItem(index + 1)}
                                            <div className={`itemFooter ${that.getCurrentRenderData(index + 1).length > 0 ? '' : 'footerHide'}`}>
                                                {PermissionManage.hasFuncPermission('/dt_calssification/def/biz/add') && (
                                                    <div onClick={this.openAddDrawer.bind(this, index)}>
                                                        <PlusOutlined />
                                                        添加
                                                    </div>
                                                )}
                                                {PermissionManage.hasFuncPermission('/dt_calssification/def/biz/sort') && (
                ...
                <CategoryDetailDrawer type='business' ref={(dom) => (this.categoryDetailDrawer = dom)} />
