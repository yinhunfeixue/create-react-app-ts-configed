...
export default class searchBlock extends Component {
    ...
    onFocus = async (e) => {
        ...
        if (arr.length > 0 && arr[length].text.length < 1) {
    ...
    setItem = async (text, type) => {
        ...
        arr.map((value, index) => {
            if (index < nodePos) {
                prePos += value.text.length
            }
        })
        let itemValue1 = arr[nodePos].text ? arr[nodePos].text.substring(0, mousePos) : ''
        let itemValue2 = arr[nodePos].text ? arr[nodePos].text.substring(mousePos, arr[nodePos].text.length) : ''
        let value = ''
        if (type === 1) {
            value = text + '()'
            // arr[nodePos].text = itemValue1 + value + itemValue2
            arr.splice(nodePos + 1, 0, { text: value })
            ...
            if (!re.test(text) || re2.test(text) || matchArr.findIndex((val) => val === text.toLocaleUpperCase()) > -1) {
                text = '`' + text + '`'
            }
            value = text
            arr.splice(nodePos + 1, 0, { text: value })
    ...
    deleteBlank = async (index, isBlank, isFoucus) => {
        ...
        if (isBlank) {
            if (arr.length < 1) {
                ...
                store.setMouseInf(index - 1, searchItem[index - 1].text.length)
                await store.setSearchItem(arr, true)
            }
            return
        }
        if (arr.length < 1) {
            ...
            await store.setSearchItem(arr, true)
            store.setMouseInf(index - 1, searchItem[index - 1].text.length)
            this.cursorPosition(index - 1, searchItem[index - 1].text.length)
            // if (!isFoucus) {
    ...
    // 固定定位到选项后
    setOptionPos = (nodePos, pos) => {
        var range = window.getSelection()
        range.selectAllChildren(this['child' + nodePos].input)
        range.collapseToEnd()
        store.setMouseInf(nodePos, pos)
    }
    onKeyDown = async (e) => {
                    ...
                    {searchItem.map((value, index) => {
                        return (
                            <SearchBlock
                                ...
                                setOptionPos={this.setOptionPos}
                                isMerge={isMerge}
                                params={value}
                            />
                        )
