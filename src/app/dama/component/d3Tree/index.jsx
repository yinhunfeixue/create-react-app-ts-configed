...
export default class d3Tree extends Component {
                        ...
                        new go.Binding("text", "name")),
                    {
                        toolTip:                       // define a tooltip for each node
                            $(go.Adornment, "Spot",      // that has several labels around it
                                { background: this.props.from == 'classTree'?"rgba(0, 0, 0, 0.65)":'transparent' },  // avoid hiding tooltip when mouse moves
                                $(go.Placeholder, { padding: 5 }),
                                $(go.TextBlock,
                                    { alignment: go.Spot.TopLeft, alignmentFocus: go.Spot.Left, stroke: this.props.from == 'classTree'?"white":'transparent', margin: 10 },
                                    new go.Binding("text", "name", function(s) { return "节点名称: " + s; })),
                                $(go.TextBlock,
                                    { alignment: go.Spot.MiddleLeft, alignmentFocus: go.Spot.Left, stroke: this.props.from == 'classTree'?"white":'transparent', margin: 10 },
                                    new go.Binding("text", "businessDepartName", function(s) { return "业务归口部门: " + s; })),
                                $(go.TextBlock,
                                    { alignment: go.Spot.BottomLeft, alignmentFocus: go.Spot.Left, stroke: this.props.from == 'classTree'?"white":'transparent', margin: 10 },
                                    new go.Binding("text", "technicalDepartName", function(s) { return "技术归口部门: " + s; }))
                            )  // end Adornment
                    }
                ),
    ...
    getTreeData = (data) => {
        ...
        data.map((item) => {
            ...
            nodeDataArray.push({ key: item.id, name: item.name, parent: item.parentId, color: color, fig: fig, businessDepartName: item.businessDepartName, technicalDepartName: item.technicalDepartName })
