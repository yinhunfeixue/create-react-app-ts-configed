...
class Table extends React.Component {
  ...
  onColumnResizeEnd = (columnWidth, cursorDelta, dataKey, index) => {
    ...
    this.scrollbarX.onWheelScroll(this.resizeScrollbarX);
    this.handleWheel(this.resizeScrollbarX, 0);
  ...
  renderTableBody(bodyCells, rowWidth) {
    ...
    if (data && data.length > 0) {
      rows = data.map((rowData, index) => {
        ...
        if(hasSumRow&&index===0){
            return (
                <div className='relation_table_total' key={index}>
                {row}
                </div>
            );
        }
