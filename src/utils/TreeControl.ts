...
class TreeControl<T extends { [key: string]: any }> {
    ...
    private innerListTree(list: T[], equalParent: (node: T, parentNode: T | null) => boolean, parent: T | null): T[] {
        if (!list) {
            return []
        }
     ...
     */
    map<W>(tree: T[], mapFunction: (node: T, index: number, oldParent: T | null, newChildren: W[] | undefined) => W) {
     ...
     * @private
     */
    _mapInner<W>(tree: T[], mapFunction: (node: T, index: number, oldParent: T | null, newChildren: W[] | undefined) => W, parent: T | null = null) {
     ...
     */
    private _searchChainInner(tree: T[], equalFunction: equalFunctionType<T>, parent: T | null = null): T[] | null {
        if (tree) {
     ...
     * @private
     */
    private _forEachInner(tree: T[], forEachFunction: (node: T, index: number, parentNode: T | null) => void, parent = null) {
        if (tree) {
            //遍历结点，对结点执行操作，并递归子结点
            for (let i = 0; i < tree.length; i++) {
                let node = tree[i]
